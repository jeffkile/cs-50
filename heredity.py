...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    """
    # people = {}
    # Keys are names, Values are dictionaries that contain mother and father keys
    # Assume either mother and father are both blank, or they are names of other keys in the people dictionary

    # Multiply the gene and trait probabilities together


    total_running_probability = 1

    # For each person
    for person in people:
        individual_probability = 1

        # If this is a child so we need to calculate probability of having the genes from the parents
        if people[person]['mother'] and people[person]['father']:

            prob_gene_from_mom= PROBS['mutation']
            if people[person]['mother'] in one_gene:
                prob_gene_from_mom = 0.5
            elif people[person]['mother'] in two_genes:
                prob_gene_from_mom = 1 - PROBS['mutation']

            prob_gene_from_dad = PROBS['mutation']
            if people[person]['father'] in one_gene:
                prob_gene_from_dad = 0.5
            elif people[person]['father'] in two_genes:
                prob_gene_from_dad = 1 - PROBS['mutation']

            if person in two_genes:
                individual_probability = individual_probability * prob_gene_from_mom * prob_gene_from_dad
            elif person in one_gene:
                individual_probability = individual_probability * prob_gene_from_mom * (1 - prob_gene_from_mom) * prob_gene_from_dad * (1 - prob_gene_from_dad)
            else:
                individual_probability = individual_probability * (1 - prob_gene_from_mom) * (1 - prob_gene_from_dad)
        ...
        # Otherwise this is a parent so we can use the PROBS tables for everything
        else:
        ...
        # Multiply each person's probability into the total running probability    
        total_running_probability = total_running_probability * individual_probability

    return total_running_probability
...
def normalize(probabilities):
    ...
    # Divide by the sum to normalize to 1
    for person in probabilities:
